...
class ChiefSentryServiceProvider extends ServiceProvider {
	...
	protected function registerHasher()
	{
		...
		{
			$hasher = $app['config']['chief::config.sentry.hasher'];
	...
	protected function registerUserProvider()
	{
		...
		{
			$model = $app['config']['chief::config.sentry.users.model'];
			...
			{
				$loginAttribute = $app['config']['chief::config.sentry.users.login_attribute'];
			...
			{
				$groupModel = $app['config']['chief::config.sentry.groups.model'];
			...
			{
				$pivotTable = $app['config']['chief::config.sentry.user_groups_pivot_table'];
	...
	protected function registerGroupProvider()
	{
		...
		{
			$model = $app['config']['chief::config.sentry.groups.model'];

			// Define the User model to use for relationships.
			if (method_exists($model, 'setUserModel'))
			{
				$userModel = $app['config']['chief::config.sentry.users.model'];
	...
	protected function registerThrottleProvider()
	{
		...
		{
			$model = $app['config']['chief::config.sentry.throttling.model'];

			$throttleProvider = new ThrottleProvider($app['chief.sentry.user'], $model);

			if ($app['config']['chief::config.sentry.throttling.enabled'] === false)
			{
			...
			{
				$attemptLimit = $app['config']['chief::config.sentry.throttling.attempt_limit'];
			...
			{
				$suspensionTime = $app['config']['chief::config.sentry.throttling.suspension_time'];
	...
	protected function registerSession()
	{
		...
		{
			$key = $app['config']['chief::config.sentry.cookie.key'];
